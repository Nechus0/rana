...
function umfang(p: Profile): string {
  ...
  return [
    ...
    "  Abschnitt 1: rund 2.900 Zeichen (etwa 370 Wörter) in sechs Absätzen, davon der letzte der Bilanzabsatz.",
    "  Abschnitt 2: rund 800 Zeichen (etwa 100 Wörter), verteilt auf die zwei bis drei Beschriftungen.",
    "  Abschnitt 3: rund 1.300 Zeichen (etwa 165 Wörter): Begründung, drei bis vier Behandlungsziele, Methodik und Setting, Prognose.",
...
function aufbau(p: Profile): string {
  ...
  return [
    ...
    "  (a) Ausgangslage zu Behandlungsbeginn: Symptomatik und Funktionsniveau, ggf. Medikation.",
    ...
    "  (c) Als LETZTER Absatz eine Bilanz mit dem Label „Zusammenfassung: “ – darin knapp, was sich bei Symptomatik und Funktionsniveau gebessert hat und was offen bleibt. WICHTIG: Nach dem Doppelpunkt beginnt ein vollständiger, eigenständiger Satz, der ohne das Label lesbar ist – also „Zusammenfassung: Es zeigt sich eine deutliche Besserung …“ und NICHT „Zusammenfassend zeigt sich …“. Das gilt für alle Beschriftungen.",
    "",
    ...
    "  (d) Ein Absatz „Prognose: “ – günstige Faktoren, Veränderungshindernisse, geplanter Abschluss. Dieser Absatz endet mit dem Satz: „Die geplante Behandlung ist damit ausreichend begründet und erfolgversprechend.“",
  ].join("\n");
}
...
export function systemPrompt(p: Profile): string {
  ...
  return [
    ...
    "1. Bisheriger Behandlungsverlauf seit dem letzten Bericht",
...
export function userPrompt(f: Felder, p: Profile): string {
  const zeilen = [
    ...
    "Prognose / geplanter Abschluss (zu Punkt 3): " + leer(f.f_prognose),
    "",
  ...
  zeilen.push(
    "",
    `ZUM SCHLUSS NOCHMALS DER UMFANG: rund ${L.ziel_soll.toLocaleString("de-DE")} Zeichen, mindestens ${L.ziel_min.toLocaleString("de-DE")}, höchstens ${L.ziel_max.toLocaleString("de-DE")}. Abschnitt 1 hat sechs Absätze. Ziele an das obere Ende, damit die zweite Seite gefüllt ist, aber strecke nichts ohne Grundlage. Zähle nach, bevor du antwortest.`
  );
